...
/*******************************************************
		TIME-COUNTING - Отсчет времени
*******************************************************/
$(function() {	
	var ar_interval_tasks = new Array();
	var ar_interval_task = new Array();
	var ar_interval_task_list = new Array();

	init('.b-tasks-timer__el span:last-child', function() {
		clearAllInterval(ar_interval_task_list);		
		$('.b-tasks-timer__el span:last-child').each(function(){
			ar_interval_task_list.push(initTimer($(this)));
		});		
	});
	init('.b-tasks__container', function() {
		clearAllInterval(ar_interval_tasks);		
		$('.b-tasks-list-el__time').each(function(){
			ar_interval_tasks.push(initTimer($(this)));
		});		
	});
	init('.b-task__value_time', function() {
		clearAllInterval(ar_interval_task);		
		$('.b-task__value_time').each(function(){			
			ar_interval_task.push(initTimer($(this)));
		});	
	});
	function clearAllInterval(ar_interval) {
		var i = ar_interval.length;
		while(--i >= 0) {
			clearInterval(ar_interval[i]);
		}		
		ar_interval.splice(0, ar_interval.length);
	}
	function initTimer(obj) {	
		var interval = setInterval(function(e) {			
			obj.html(getСounter(obj));
		}, 1000);
		return interval;
	}
	function getСounter(obj) {
		var ar_time = obj.html().split(':');
		var timer = (ar_time[0] * 3600) + (ar_time[1]*60) + (ar_time[2] - 1);	
		if(timer < 0) timer = 0;
...
/*******************************************************
		TASKS - Задания
*******************************************************/
init('.b-tasks-list__container',function() {
	var conainer_pos = $('.b-tasks-list__container').position().left;
	var animation = false;
	
	$('.b-tasks__left').on('click', function() {	
		if(animation == true)
			return;
			
		var skip = false;
		$('.b-tasks-list__el').each(function(index){
			if(skip == true)
				return;
			if($(this).hasClass('b-tasks-list__el_current')) {
				if(index > 0) {	
					$(this).removeClass('b-tasks-list__el_current');
					var prev_obj = $($('.b-tasks-list__el')[index-1]);
					prev_obj.addClass('b-tasks-list__el_current');
					anim(prev_obj.position().left);
					skip = true;
				}
			}
		});
	});
	$('.b-tasks__right').on('click', function(){
		if(animation == true)
			return;
		
		var max_index = $('.b-tasks-list__el').length - 1;
		var skip = false;
		$('.b-tasks-list__el').each(function(index){
			if(skip == true)
				return;
			...
			if($(this).hasClass('b-tasks-list__el_current')) {				
				if(index != max_index) {	
					$(this).removeClass('b-tasks-list__el_current');
					var next_obj = $($('.b-tasks-list__el')[index+1]);
					next_obj.addClass('b-tasks-list__el_current');
					anim(next_obj.position().left);
					skip = true;
				}
			}
		});
	...
	function anim(pos) {		
		animation = true;				
		$('.b-tasks-list__container').animate({
			'margin-left': -pos,
		},500, function(){
			animation = false;
		});
...
*******************************************************/
init('.b-game-panel-size', function(obj){
...
*******************************************************/
init('.b-detail-element__game_unity3d', function(obj) {
	var file_url = obj.data('file_url');
	var width = obj.data('width');
	var height = obj.data('height');
	var game_container = obj.data('game_container');
	var ads_container = obj.data('ads_container');	
	unity3d_container(file_url, width, height, game_container, ads_container);
});
/*******************************************************
		READY GAME-FLASH - ГОТОВНОСТЬ FLASH ИГРЫ
*******************************************************/
init('.b-detail-element__game_flash', function(obj) {
	var file_url = obj.data('file_url');
	var width = obj.data('width');
	var height = obj.data('height');
	var game_container = obj.data('game_container');
...
function setPage(page) {
	...
	$('.b-tasks').html($(page).find('.b-tasks').html());
}
function setPageForUpdate(page) {
	$('.b-tasks').html($(page).find('.b-tasks').html());
}
/*******************************************************
